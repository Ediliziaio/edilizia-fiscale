...
import { BookOpen, ArrowRight, Search, X, HelpCircle, BookMarked } from "lucide-react";
    ...
    <>
      ...
      <div className="min-h-screen bg-background flex flex-col">
        ...
        <main className="flex-1">
          ...
          {/* Le sei aree come pagine vere: i filtri qui sopra sono client-side,
              questi sono i link che un crawler segue per arrivare ai cluster. */}
          {showPillars && (
            <section className="py-12 lg:py-14 bg-white border-b border-border">
              <div className="container mx-auto px-4">
                <Reveal>
                  <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-ink leading-tight mb-2">
                    Le sei aree, una per volta
                  </h2>
                  <p className="text-foreground/70 mb-7 max-w-2xl leading-relaxed">
                    Ogni area ha una pagina con il quadro d'insieme e l'elenco completo delle sue guide.
                  </p>
                </Reveal>
                  ...
                  {categorie.map((c, i) => (
                    <Reveal key={c.slug} delay={(i % 3) * 60}>
                      <Link
                        ...
                        className="group block h-full bg-muted/40 rounded-2xl border border-border p-6 hover:border-ink hover:bg-white transition-colors"
                      >
                        <div className="flex items-baseline justify-between gap-3 mb-2">
                          <h3 className="font-bold text-ink text-lg leading-snug group-hover:text-brand-dark">
                            {c.label}
                          </h3>
                          <span className="text-sm text-foreground/45 tabular-nums shrink-0">
                            {counts[c.categoria] ?? 0}
                          </span>
                        </div>
                        <p className="text-sm text-foreground/70 leading-relaxed line-clamp-3">{c.intro}</p>
                        <span className="mt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-brand-dark group-hover:gap-2.5 transition-all">
                          Apri l'area <ArrowRight className="w-4 h-4" />
                        </span>
                      </Link>
                    </Reveal>
                  ))}
