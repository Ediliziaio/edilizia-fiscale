...
export const article: Article = {
  ...
  content: [
    ...
    { type: "p", text: "La terza riga richiede la massima attenzione ed è quella in cui l'affitto viene usato peggio. Un affitto d'azienda a canone incongruo, verso una società riconducibile agli stessi soci, stipulato quando i creditori sono già alla porta, è un'operazione fragile: espone all'[azione revocatoria](/glossario#azione-revocatoria) e, in certe condizioni, ad altre valutazioni. Non significa che l'affitto in crisi sia vietato — è previsto e utilizzato anche nei percorsi di risanamento — significa che va fatto con canone di mercato, ragioni documentate e nel quadro giusto." },
    { type: "p", text: "La prima riga è invece l'uso più interessante e meno praticato. In un [passaggio generazionale](/guide/passaggio-generazionale-impresa-edile) l'affitto consente ai figli di guidare l'impresa per un periodo definito, con il fondatore che resta proprietario e osserva. Se funziona si procede con il trasferimento; se non funziona, il contratto scade e l'azienda torna. È una prova reversibile che una donazione di quote non consente." },
