...
const ArticleTimeline = ({ title, steps }: ArticleTimelineProps) => (
  <div className="my-8 bg-muted/50 border border-border rounded-2xl p-6 lg:p-7">
    {title && (
      <h3 className="text-base font-bold text-ink mb-5 uppercase tracking-wide">{title}</h3>
    )}
    <ol className="relative space-y-5">
      {/* filo verticale che unisce i pallini */}
      <span
        className="absolute left-[7px] top-2 bottom-2 w-px bg-gradient-to-b from-brand via-brand/40 to-transparent"
        aria-hidden="true"
      />
      {steps.map((s, i) => (
        <li key={i} className="relative pl-8">
          <span
            className="absolute left-0 top-1.5 w-[15px] h-[15px] rounded-full bg-brand ring-4 ring-muted/50"
            aria-hidden="true"
          />
          <span className="block text-xs font-bold uppercase tracking-wider text-brand-dark mb-0.5">
            {s.when}
          </span>
          <span className="block font-semibold text-ink leading-snug">{s.label}</span>
          {s.detail && (
